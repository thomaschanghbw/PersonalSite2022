...
so trivial a tradeoff to either put on a mask or have their journey to the store be for nothing.
...
> Edit: I realize that people's experiences with mask mandates varies tremendously by their city dynamics, so it would be
> helpful to set some context: I come from a city with very split (almost exactly 50/50) politics, meaning there are
> many people eager to enforce mask mandates, and equally many people who hate them.
